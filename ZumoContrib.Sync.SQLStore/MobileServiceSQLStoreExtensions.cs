...
{
    /// <summary>
    ///  Provides extension methods on <see cref="MobileServiceSqlStore"/>.
    /// </summary>
    public static class MobileServiceSqlStoreExtensions
    {
        public static void DefineTable<T>(this MobileServiceSqlStore store)
        {
            var settings = new MobileServiceJsonSerializerSettings();
            DefineTable<T>(store, settings);
        }

        public static void DefineTable<T>(this MobileServiceSqlStore store, MobileServiceJsonSerializerSettings settings)
        {
            ...
            if (contract == null)
            {
                throw new ArgumentException(Properties.Resources.SqlStore_DefineTableTNotAnObject);
                
            }
            if (contract.DefaultCreator == null)
            {
                throw new ArgumentException(Properties.Resources.SqlStore_DefineTableEmptyCtorNotDefined);
